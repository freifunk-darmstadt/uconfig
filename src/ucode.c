...
static uc_resource_type_t *uconfig_type;
struct uconfig_context *ctx = NULL;
 ...
 */
static int
uc_context(struct uconfig_context *ctx)
{
	/* check if the global object exists */
	uc_value_t *obj = ucv_object_get(uc_vm_scope_get(ctx->vm), "uconfig", NULL);
	if (!obj) {
		ULOG_ERR("failed to load global.uconfig\n");
		return -1;
	}
...
uc_connect(uc_vm_t *vm, size_t nargs)
{
	...
	return uc_resource_new(uconfig_type, ctx);
}
...
{
	struct uconfig_context **ctx = uc_fn_this("uconfig.context");
...
static const uc_function_list_t uconfig_fns[] = {
	{ "send",	uc_send },
...
{
	struct uconfig_context *ctx = (struct uconfig_context *) ud;
...
{
	uconfig_type = uc_type_declare(vm, "uconfig.context", uconfig_fns, uc_context_close);

...
lws_protocols ws_protocols[] = {
	{ "device", ws_callback, 0, 32 * 1024, 0, NULL, 0},
	{ NULL }
};

int
ws_connect(struct uconfig_context *_ctx)
{
	...
	info.client_ssl_cert_filepath = "/etc/uconfig/device/cert.pem";
	info.client_ssl_private_key_filepath = "/etc/uconfig/device/key.pem";
	info.client_ssl_ca_filepath = "/etc/uconfig/device/ca.pem";
